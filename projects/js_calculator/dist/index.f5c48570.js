...
})({"5zLov":[function(require,module,exports) {
"use strict";
...
/* Setting window height for calculator */ let vh = window.innerHeight * 0.01;
document.documentElement.style.setProperty("--vh", `${vh}px`);
window.addEventListener("resize", ()=>{
    window.innerHeight;
    document.documentElement.style.setProperty("--vh", `${vh}px`);
});
/* Initializing const for calculator inputs and outputs */ const calcInputs = new Map([
    [
        0,
        "Del"
    ],
...
/* Main Grid for Calculator Buttons */ const makeGrid = (rows, cols)=>{
...
/* Helper functions for styling, event listening, and button operations */ const specBtnListener = (elem, inp)=>{
    const opacVals = [
        "0.5",
        "0.6",
        " 0.3",
    ...
    const addEvents = (colorInp)=>{
        ...
        for(let i = 0; i < eventTypes.length; i++)elem.addEventListener(eventTypes[i], (event)=>{
            event.target.style.backgroundColor = `rgba(${colorInp},${opacVals[i]})`;
        });
    };
    if (inp === "CE") addEvents(colorSchemeColor[0]);
    else if (inp === "=") addEvents(colorSchemeColor[1]);
};
/* Main Operations Function */ const checkOperations = (index, inpType)=>{
    const symbols = [
        "+",
        "-",
        "/",
    ...
    let inp;
    if (inpType == "click") inp = calcInputs.get(index);
    else inp = index;
    if (numInput.innerText === "0") switch(true){
        case [
            ...symbols
        ].includes(inp):
        ...
        ...symbols
    ].includes(numInput.innerText.slice(-1))) switch(inp){
        case "CE":
            initializeNums();
            break;
        case "Del":
...
/* Secondary Operations Functions */ const displayOutput = (displayedInp)=>{
    if (displayedInp.search(/[\(\)]/) !== -1 && displayedInp.match(/\(/g).length !== displayedInp.match(/\)/g).length) return;
    displayedInp = displayedInp.replace(/x/g, "*").replace(/\^/g, "**").replace(/\π/g, "Math.PI").replace(/e/g, "Math.E");
    if (displayedInp.search(/a/g) !== -1) displayedInp = displayedInp.replace(/abs/g, "Math.abs");
    const result = Math.round(10 ** 14 * Function("return " + displayedInp)()) / 10 ** 14;
    if (result > 10 ** 14) numOutput.innerText = result.toExponential(7);
    else numOutput.innerText = result;
};
const delFunc = (displayedInp)=>{
    if (displayedInp.length === 1) numInput.innerText = "0";
    else numInput.innerText = displayedInp.slice(0, displayedInp.length - 1);
};
...
makeGrid(5, 5);
initializeNums();
/* Keyboard Listener */ document.addEventListener("keydown", (event)=>{
    switch(event.key){
        case "0":
        case "1":
        case "2":
...
},{}]},["5zLov","8ZNvh"], "8ZNvh", "parcelRequire7e51")
