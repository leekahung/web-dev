...
export default function Layout({ children }: Props) {
  ...
  return (
    <div
      className="dark:text-slate-200 relative h-dvh w-screen flex items-center justify-center bg-slate-200 dark:bg-slate-800 transition-all duration-1000 opacity-50 blur-3xl"
      ref={pageRef}
    >
      <header>
      ...
      <main className="flex flex-col w-[80%] min-w-[300px] max-w-[500px] gap-4 md:gap-8">
        {children}
      </main>
      <footer className="fixed flex flex-col items-center gap-4 bottom-5 z-10">
        <div className="font-light">&#169; 2025 Ka Hung Lee</div>
      </footer>
      <div
        ref={blobRef}
        className={`fixed h-40 w-40 rounded-full pointer-events-none blur-3xl opacity-20
